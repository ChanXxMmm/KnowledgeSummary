...
* synchronized锁优化(锁可以升级但不能降级，目的是为了提高获得锁和释放锁的效率)
  * 偏向锁: 大多数情况下锁不仅不存在多线程竞争，而且总是由同一线程多次获得，为了让线程获得锁的代价更低而引入了偏向锁，减少不必要的CAS操作
    * 线程访问同步块，访问Mark Word中偏向锁的标识是否设置成1，锁标志位是否为01，确认为可偏向状态
    * 检查对象头中是否存储了该线程id，如果没有，则直接通过CAS操作将对象头中的线程id指向自己，然后执行同步块，该线程如果再次访问同步块，检查对象头中是否存储了该线程id，如果是，则不再通过CAS，直接执行同步块
    * 检查对象头中时，如果存储的是别的线程id，则通过CAS操作竞争锁，如果成功则入上步所说，如果没有成功，说明有竞争，当到达全局安全点（safepoint）时获得偏向锁的线程被挂起，偏向锁升级为轻量级锁，然后被阻塞在安全点的线程继续往下执行同步代码。（撤销偏向锁的时候会导致stop the word，因为新线程要去修改拿到锁线程的堆栈信息将偏向锁改为轻量级锁）
  * 轻量级锁: 轻量级锁是由偏向锁升级来的，偏向锁运行在一个线程进入同步块的情况下，当第二个线程加入锁争用的时候，偏向锁就会升级为轻量级锁
    * 线程访问同步块，访问Mark Word中偏向锁的标识是否设置成0，锁标志位是否为01，确认为无锁状态且不允许进行偏向
    * 虚拟机首先将在当前线程的栈帧中建立一个名为锁记录（Lock Record）的空间，用于存储锁对象目前的Mark Word的拷贝，然后拷贝对象头中的Mark Word复制到锁记录中
    * 拷贝成功后，虚拟机将使用CAS操作尝试将对象的Mark Word更新为指向Lock Record的指针，并将Lock record里的owner指针指向对象的mark word
    * 如果CAS成功，那么这个线程就拥有了该对象的锁，并且对象Mark Word的锁标志位设置为“00”，即表示此对象处于轻量级锁定状态
    * 如果CAS失败，虚拟机首先会检查对象的Mark Word是否指向当前线程的栈帧，如果是就说明当前线程已经拥有了这个对象的锁，那就可以直接进入同步块继续执行
    * 如果没有指向自己，说明多个线程竞争锁，那么它就会自旋等待锁，一定次数后仍未获得锁对象。重量级线程指针指向竞争线程，竞争线程也会阻塞，等待轻量级线程释放锁后唤醒他。锁标志的状态值变为“10”，Mark Word中存储的就是指向重量级锁（互斥量）的指针，后面等待锁的线程也要进入阻塞状态
  * 自适应自旋锁: 自旋的时间不在是固定的了，而是由前一次在同一个锁上的自旋时间以及锁的拥有者的状态来决定，基本认为一个线程上下文切换的时间是最佳的一个时间
  * 锁消除: 虚拟机的运行时编译器在运行时如果检测到一些要求同步的代码上不可能发生共享数据竞争，则会去掉这些锁
  * 锁粗化: 将临近的代码块用同一个锁合并起来
  * 逃逸分析: 如果这个对象不会逃出这个方法外，则虚拟机会对其做优化
...
* volatile在DCL(多用于[单例](../../设计模式/单例模式.md))上的作用: 我们在new 对象的时候可以理解为三步
  * 内存中分配空间
  * 空间初始化
  * 把这个空间地址给到我们的引用 

如果没有防指令重排序则会出现上面三步中最后一步排在中间，而此时如果再来一个线程去请求这个对象，发现有引用，但是其实对象实例初始化工作还没做完，导致空指针异常，所以加入volatile就可以防止这个问题
