...
//ThreadLocalMap
 static class Entry extends WeakReference<ThreadLocal<?>> 
```
我们知道ThreadLocalMap中的Entry是用的弱引用(对对象的引用不了解可以去[JVM](../../JVM/JVM.md)中对象的引用中学习)来保存ThreadLocal<?>，而每个线程中都有一个ThreadLoaclMap保存ThreadLocal<?>和value，当发生垃圾回收时，ThreadLocal<?>就被回收，但是value此时还依旧保存在ThreadLocalMap中，这就是导致内存泄漏的原因，(虽然每次调用get的时候都会将map中key被回收了的value清除)，所以每次使用完Threadlocal时一定要调用它的remove方法(从ThreadLocalMap中清除该线程的ThreadLoacl<?>)
