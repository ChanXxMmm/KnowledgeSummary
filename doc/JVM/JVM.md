...
# 虚拟机栈 :
  ...
  * 虚拟机栈内部包含栈帧
    ...
    3. 动态链接: Java语言特性多态，需要类运行时才能确定具体的方法
    ```java
    ...
    4. 完成出口: 可以理解为返回地址，比如上面的demo中说的work方法执行完毕，就会打包成一个栈帧携带者z值出栈，此时就会有一个返回地址，也就是方法出口。而具体地址如果方法正常返回则使用程序计数器中的地址作为返回地址，如果异常则要去异常处理表来进行确定
