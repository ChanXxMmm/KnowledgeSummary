...
* 多线程并发收集器: 在清理中，可以不暂停用户线程，会将用户线程和回收线程共享一部分，但不是说全都不暂停和全程暂停，只是部分的时间，用户线程和回收线程可以同时工作(并发)
  ...
  * G1: 
    * 内存布局: 它会针对整体堆内存进行重新划分，不再是一个新生代和一个老年代，它会将整体分成很多的等份，有的区域可能是Eden区，Survivor区，Old区，或者大对象区(超过同等份单个大小的一半就认为是大对象)， 
    * 算法: Eden和Survivor区还是复制算法，老年代是标记整理
    * 筛选回收: 相比于CMS，它也会进行初始标记，并发标记，最终标记，只有最后会有一次筛选回收，由于追求停顿时间，所以会通过前面的步骤筛选那些回收效率高的对象
