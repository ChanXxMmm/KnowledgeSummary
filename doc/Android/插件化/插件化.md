...
由于第一种方法需要修改aapt源码，所以我们我们只看第二种方案：专门创建一个AssetManager加载插件的资源
```java
...
```
使用时直接在插件的Activity(继承Activity)中重写getResources
```java
    @Override
...
# 如何解决冲突
我们可以看到如果上面的插件Activity继承自AppCompatActivity就会出现冲突问题，会报一个空指针异常，找不到decor_content_parent这个系统资源，那这个问题是怎么来的呢？我们可以自己尝试下看看这两个apk中关于这个资源id，会发现他们是不一样的，所以插件的底层去拿decor_content_parent这个系统的时候，其实拿的是宿主的，所以肯定会有问题，更简单的说就是因为插件中用的是宿主的Context，导致这个问题的出现，当然如果继承Activity可能也会出现问题，只能说成功是因为运气好，两个id都是相同的

那如何解决呢？我们只需要让插件自己创建一个Context，同时跟启动插件资源的Resource进行绑定即可，所以我们上面的重写getResources的方式就不使用了，直接使用下面的方式
```java
 @Override
...
```
之后插件中要使用资源都是通过这个mContext来使用即可,比如
```java
