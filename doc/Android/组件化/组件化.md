...
- [3.APT](#APT)
- [4.手写类似ARouter](#手写类似ARouter)
- [5.手写类似ButterKnife](#手写类似ButterKnife)
- [6.ARouter解析](#ARouter解析)
...
//        String value = processingEnvironment.getOptions().get("hi");
    ...
    private final boolean checkRouterPath(RouterBean bean){
        ...
        //返回false执行一次,true的话执行一遍，检查一遍
        return true;
    }
}
```

此时我们发现我们通过此方法只是能通过方法拿到目标Activity，但是还要自己去写跳转传参等逻辑，所以我们需要一个路由管理类
```java
public class RouterManager {
    ...
    /***
     * @param path 例如：/app/MainActivity
     *      * @return
     */
     //BundleManager主要负责参数传递，后面会说
    public BundleManager build(String path) {
        if (TextUtils.isEmpty(path) || !path.startsWith("/")) {
            throw new IllegalArgumentException("不按常理出牌 path乱搞的啊，正确写法：如 /app/MainActivity");
        }
        
        //path校验只写一个，其他的可参考上面APT中的校验
        if (path.lastIndexOf("/") == 0) { // 只写了一个 /
            throw new IllegalArgumentException("不按常理出牌 path乱搞的啊，正确写法：如 /app/MainActivity");
        }

        // 截取组名 /app/MainActivity  finalGroup=app
        String finalGroup = path.substring(1, path.indexOf("/", 1)); // finalGroup = app
        ...
        // 证明没有问题，没有抛出异常
        this.path =  path;  // 最终的效果：如 /app/MainActivity
    ...
    public Object navigation(Context context, BundleManager bundleManager) {
        ...
        try {
            ...
            if (null == loadPath) { // 缓存里面没有东东 Path
                ...
                // 3.从map里面获取 ARouter$$Path$$app.class
            ...
            if (loadPath != null) { // 健壮
                ...
                if (routerBean != null) {
                    switch (routerBean.getTypeEnum()) {
                        case ACTIVITY:
                            Intent intent = new Intent(context, routerBean.getMyClass()); // 例如：getClazz == MainActivity.class
...
RouterManager.getInstance()
  .build("/second/Second_MainActivity")
  .withString("name","abc")
  .navigation(this);
```
大功告成！！
...
//这就是我们要生成的方法
public class Activity名字$$Parameter implements ParameterGet { 
  @Override
  public void getParameter(Object targetParameter) {
  Activity名字 t = (Activity名字) targetParameter; 
...
@SupportedAnnotationTypes({"cn.xxx.Parameter"}) // 我们服务的注解
      ...
      if(set!=null&&set.size>0){
          ...
          Set<? extends Element> elements = roundEnvironment.getElementsAnnotatedWith(Parameter.class);
              if(elements!=null&&elements.size>0){
                ...
                // 判断是否有需要生成的类文件
                if (tempParameterMap == null || tempParameterMap.size==0 ) return true;
                
                TypeElement activityType = elementUtils.getTypeElement(android.app.Activity);
                TypeElement parameterType = elementUtils.getTypeElement(cn.xxx.ParameterGet);
                ...
                for (Map.Entry<TypeElement, List<Element>> entry : tempParameterMap.entrySet()) {
                    ...
                    ClassName className = ClassName.get(typeElement);
                     ...
                     // 生成方法，ParameterFactory后面会说，就是一个构建者模式来创建方法，但是此时方法没有内容
                    ParameterFactory factory = new ParameterFactory.Builder(parameterSpec)
                            .setMessager(messager)
                            .setClassName(className)
                            .build();
                            
                    //通过ParameterFactory生成Activity名字 t = (Activity名字) targetParameter; 这句话
                    factory.addFirstStatement();        
                     
                    //通过类中使用的注解，自动根据属性的类型生成不同的getIntent.getInt/getString等
                    for (Element element : entry.getValue()) {
                        factory.buildStatement(element);
                    }
                    
                    //类名： Activity名字$$Parameter
...
public class ParameterFactory {
    ...
    /** 只有一行
     * Activity t = (Activity) targetParameter;
     */
    ...
    public void buildStatement(Element element) {
        ...
        annotationValue = TextUtils.isEmpty(annotationValue) ? fieldName : annotationValue;
        ...
        // 健壮代码
        if (methodContent.endsWith(")")) { // 全部的 getBooleanExtra  getIntExtra   getStringExtra
...
```
通过添加注解，编译后就自动生成了文件，但是此时我们还需要一个管理类去帮助我们通过当前Activity去调用我们生成的代码类和方法
```java
public class ParameterManager {
    ...
    // 为什么还要拼接，此次拼接 是为了寻找到我们的类名
    static final String FILE_SUFFIX_NAME = "$$Parameter"; 

    // 使用者 只需要调用这一个方法，就可以进行参数的接收
    public void loadParameter(Activity activity) { // 必须拿到Activity
        ...
        if (null == parameterLoad) { // 缓存里面没东东   提高性能
            // 拼接 如：MainActivity + $$Parameter
            // 类加载MainActivity + $$Parameter
            try {
                // 类加载MainActivity + $$Parameter
                Class<?> aClass = Class.forName(className + FILE_SUFFIX_NAME);
                // 用接口parameterLoad = 接口的实现类MainActivity
...
# ARouter解析
根据上面的东西，其实ARouter的思想也差不多，不过它的实现更加的丰富，我们可以从它的初始化地方开始分析
```java
...
public static void init(Application application) {
        if (!hasInit) { //确保只初始化一次
            ...
            //Arouter是对外暴露api的类，而_ARouter是真正的实现类
...
public synchronized static void init(Context context, ThreadPoolExecutor tpe) throws HandlerException {
    ...
    if (ARouter.debuggable() || PackageUtils.isNewVersion(context)) {
        //从apt生成的包中加载类到集合中，因为ARouter都是加载到com.alibaba.android.arouter.routes这个目录下，所以就是遍历这个目录加载到集合内
        //这个过程很耗时，所以在1.3后通过字节码插庄通过线程池执行
    ...
    //判断类型，使用反射实例化对象，并调用方法，等于是将Root，Interceptors和Providers的类实例化，并调用loadInto方法，将对应注解的类加到不同的indexmap中
...
```
我们再来看下之前所说的afterInit方法
```java
static void afterInit() {
    //它是一个管理拦截器的服务，实现了IProvider接口，并且用自己实现的的路由方式获取实例
    interceptorService = (InterceptorService) ARouter.getInstance().build("/arouter/service/interceptor").navigation();
}
```
IProvider接口是用来暴露服务，并且在初始化的时候会被调用init(Context context)方法。具体的服务有其实现提供，那我们就来看下它的实现做了些什么
```java
...
```
build方法最终返回一个Postcard对象。它也是一个数据bean，继承自RouteMeta，附加一些跳转信息
    ...
    if (null == routeMeta) {    // 内存中没有，可能是还没有加载过
        ...
        switch (routeMeta.getType()) {
            case PROVIDER:  //如果目标类型是provider，实例化对象并放入postcard
                ...
                break;
            case FRAGMENT:
                postcard.greenChannel();    // Fragment不经过拦截器处理
            default:
                break;
        }
    }
...
protected Object navigation(final Context context, final Postcard postcard, final int requestCode, final NavigationCallback callback) {
    ...
    if (!postcard.isGreenChannel()) {  //不是绿色通道，即经过拦截器处理
        //内部就是会创建一个与拦截器数量相同的CancelableCountDownLatch初始计数值，每放行一个拦截器就countDown，并交给后一个拦截器，如果拦截则清0计数，并将拦截的Throwable存入postcard的tag字段
...
```
navigation可以传入Context或者不传，不传的话默认就用Application的context，每开启一个Activity就会新开一个task
...
```
通过上面这句话，就可以通过@Autowired注解的属性实现自动注入
```java
    ...
    public void autowire(Object instance) {
        ...
        try {
            ...
            if (!blackList.contains(className)) {
                //ISyringe是一个接口，它的实现是apt生成的，每一个带有@Autowired注解的类都会生成一个对应的ISyringe的实现
...
```
其注入原理基本与ButterKnife类似，我们在上面手写的简单版也基本实现了类似的思想
